...
package cms // import "github.com/cxr29/aliyun-openapi-go-sdk/apis/cms/v20150729"
...
func (api API) PutMetricDatum(Metrics, Namespace string, optional openapi.M) (*PutMetricDatumResponse, error) {
	...
	args.Query.Set("Action", "PutMetricDatum")
	args.Query.Set("Metrics", Metrics)
	args.Query.Set("Namespace", Namespace)
	args.Scheme = "http"
	if v, ok := optional["ResourceOwnerId"]; ok {
		if ResourceOwnerId, ok := v.(int64); ok {
	...
	result := new(PutMetricDatumResponse)
	if err := api.Service.Do(result, args); err != nil {
		return nil, err
	}
	return result, nil
