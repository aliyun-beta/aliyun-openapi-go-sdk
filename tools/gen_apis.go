...
func main() {
	...
	for _, info := range a {
		...
		pkg := vi.Package()
		ver := "v" + strings.Replace(vi.Name, "-", "", -1)
		...
		buf.WriteString(fmt.Sprintf("package %s // import \"github.com/cxr29/aliyun-openapi-go-sdk/apis/%s/%s\"\n", pkg, pkg, ver))
...
`)
		...
		dir = filepath.Join("../apis", pkg, ver)
